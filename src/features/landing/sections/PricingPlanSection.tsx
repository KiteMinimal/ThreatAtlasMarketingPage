...
const plans: Plan[] = [
  {
    ...
    features: ["Up to 5 users", "Basic threat detection", "Email support", "Standard integrations"],
    popular: false,
  },
  {
    name: "Professional",
...
export default function PricingPlanSection() {
  return (
    <section id="pricing" className="relative py-28 px-6 md:px-10 lg:px-16 scroll-mt-20">
      ...
      <div aria-hidden className="absolute -top-24 left-0 h-64 w-64 rounded-full blur-3xl bg-gradient-to-br from-blue-600/20 to-purple-600/20 -z-10" />
      <div aria-hidden className="absolute -bottom-28 right-0 h-72 w-72 rounded-full blur-3xl bg-gradient-to-tr from-purple-600/20 to-blue-600/20 -z-10" />
      ...
      <div className="mx-auto max-w-6xl">
        ...
        >
          {plans.map(({ name, price, frequency, description, features, popular }) => {
            ...
            return (
              <div key={name} className={popular ? "md:scale-[1.02] md:-translate-y-1" : ""}>
                {Card}
              </div>
            );
          })}
